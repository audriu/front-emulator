...
* [`src/cljs/front_emulator/`](src/cljs/front_emulator/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/front_emulator/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `front-emulator` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/front_emulator/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns front-emulator.example
  (:require [front-emulator.config :as config])
